...
function plotgaps(x_values::Vector{Float64}, f_x::Vector{Float64}, h_x::Vector{Float64},  
    ...
    p = plot(x_values, f_x, label="f(x) No Shock", xlabel="Durable gap, log points, x", ylabel="Density", legend=:topright, color=:blue)
    plot!(p, x_values_shock, f_x_shock, label="f(x) Shock", color=:darkred)
