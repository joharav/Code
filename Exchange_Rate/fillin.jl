function fillin(obj::Array{Float64}, g::NamedTuple)
  ...
  objlong = zeros(sz.np, sz.ne, sz.npa, sz.npd)
        ...
        # Two-dimensional interpolation
          Threads.@threads for ie in 1:sz.ne
              Threads.@threads for ip in 1:sz.np
                  objlong[ip, ie, ia, id] = afrac * (dfrac * obj[ip, ie, aup, dup] + (1 - dfrac) * obj[ip, ie, aup, ddown]) +
                  (1 - afrac) * (dfrac * obj[ip, ie, adown, dup] + (1 - dfrac) * obj[ip, ie, adown, ddown])
              end
          end
    end
  end
