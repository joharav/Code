function tinybellman(q::Array{Float64}, pr::Array{Float64}, old_gidx::dtp.Ipol)
    ...
    global sapol = zeros(Int, sz.np, sz.ne, sz.na, sz.nd)
    global sdpol = zeros(Int, sz.np, sz.ne, sz.na, sz.nd)
    global vnew = zeros(Float64, sz.np, sz.ne, sz.na, sz.nd)
        ...
        Threads.@threads for ia in 1:sz.na
            Threads.@threads for ie in 1:sz.ne
                Threads.@threads for ip in 1:sz.np
                    ja = old_gidx.a[ip, ie, ia, id]
                    jd = old_gidx.d[ip, ie, ia, id]
                    ...
                    avsmall = beta * q[ip, ie, l_a:u_a, l_d:u_d] + pr[ip, ie, ia, id, l_a:u_a, l_d:u_d]
                    tmp = argmax(avsmall)
                    iia = tmp[1]
                    iid = tmp[2]
                    sapol[ip, ie, ia, id] = iia
                    sdpol[ip, ie, ia, id] = iid
                    vnew[ip, ie, ia, id] = avsmall[iia, iid]
                end
            end
        end
    end
