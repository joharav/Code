function tinybellman(q::Array{Float64}, pr::Array{Float64}, old_gidx::dtp.Ipol)
    ...
    global sdpol = zeros(Int, sz.ne, sz.na, sz.nd)
    ...
    Threads.@threads for id in 1:sz.nd
        Threads.@threads for ia in 1:sz.na
            Threads.@threads for ie in 1:sz.ne
                ...
                avsmall = beta * q[ie, l_a:u_a, l_d:u_d] + pr[ie, ia, id, l_a:u_a, l_d:u_d]
                tmp = argmax(avsmall)
                iia = tmp[1]
                iid = tmp[2]
                sapol[ie, ia, id] = iia
                sdpol[ie, ia, id] = iid
                vnew[ie, ia, id] = avsmall[iia, iid]
            end
        end
    end
