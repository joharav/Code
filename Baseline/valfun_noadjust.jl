function valfun_noadjust(pea::Vector{Float64})
    ...
    v = zeros(sz.np, sz.na, sz.nd)  # Start with the initial value function passed in
    ...
    grids = makegrids(pea)
    util = utility_noadjust(grids, pea)
    tmat = grids.t
    errcode = 0
    ...
    if iter <= sz.earlyiter || sum_in_a_row == 0; 
            ...
            if sum_in_a_row > 0; 
                vnew, gidx = maxbellman(queuelong,util);
            
            # Otherwise do Howard
            else;
                vnew, gidx = howard(queuelong,util,gidx);
            end;
        # If you are past earlyiter and the policy function has not yet converged.
        else;                           
            vnew, gidx = tinybellman(queuelong,util,gidx)
        end
