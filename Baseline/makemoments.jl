function makemoments(simdata::NamedTuple, pea::Vector{Float64})
    ...
    for i in 1:size(current_a, 1), j in 1:size(current_a, 2)
        if current_d[i, j] == d_next[i, j]
            c[i, j] = w + current_a[i, j] * (1 + rr) + current_p[i, j] * (1 - f) * (1 - delta) * current_d[i, j] - a_next[i, j]
        else
            c[i, j] = w + current_a[i, j] * (1 + rr) + current_p[i, j] * (1 - delta) * current_d[i, j] - a_next[i, j] - current_p[i, j] * d_next[i, j]
        end
    end
    ...
    for j in 1:sz.nFirms
        ...
        for i in max(1, sz.burnin-2):min(sz.nYears, size(d, 1))
            total_periods += 1
            if d[i, j] == last_value
                spell_length += 1
            else
