function valfun_noadjust(pea::Vector{Float64})
    # Initialize arrays
    v        = zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd)     # value function
    vnew     = zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd)     # new value function 
    mew      = zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd)     # stationary distribution
    ...
    pol = dtp.Pol(zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd), zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd))

    # Initialize and instantiate the old integer policy function
    old = dtp.Ipol(Int.(zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd)), Int.(zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd)))  
    ...
    Threads.@threads for iter in 1:sz.maxiter
        ...
        reshaped_v = reshape(v, sz.np * sz.ne * sz.nw, sz.na * sz.nd)

        # Perform matrix multiplication
        result = tmat * reshaped_v
        
        # Now reshape the result to the desired shape for 'queue'
        queue = reshape(result, sz.np, sz.ne, sz.nw, sz.na, sz.nd)
