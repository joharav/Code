function utility(grids::NamedTuple, pea::Vector{Float64})
    ...
    util = zeros(sz.np, sz.ne, sz.nw, sz.na, sz.nd, sz.npa, sz.npd)
                    ...
                    Threads.@threads for iw in 1:sz.nw
                        Threads.@threads for ie in 1:sz.ne
                            Threads.@threads for ip in 1:sz.np
                                ...
                                c = w[iw] + e[ie] * a[ia] * (1 + rr) + e[ie] * pd[ip] * (1 - f) * (1 - delta) * d[id] - ap[iia] - e[ie] * pd[ip] * dp[iid]

                                # Check feasibility of consumption and durable goods stock
                                if c > 0 && dp[iid] > 0
                                    # Calculate utility
                                    util[ip, ie, iw, ia, id, iia, iid] = (((c^nu) * (dp[iid]^(1 - nu)))^(1 - gamma)) / (1 - gamma)
                                else
                                    # Apply penalty for infeasible choices
                                    util[ip, ie, iw, ia, id, iia, iid] = -1e10
                                end
                            end
                        end
                    end
