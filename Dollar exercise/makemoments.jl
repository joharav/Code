function makemoments(simdata::NamedTuple, pea::Vector{Float64})
    ...
    e = simdata.e[sz.burnin-2:sz.nYears, :]
    w = simdata.w[sz.burnin-2:sz.nYears, :]
    ...
    for i in 1:sz.nYears-sz.burnin-2, j in 1:sz.nFirms
        if current_d[i, j] == d_next[i, j]
            c[i, j] = current_w[i, j] + current_e[i, j] * current_a[i, j] * (1 + rr) + current_e[i, j] * current_p[i, j] * (1 - f) * (1 - delta) * current_d[i, j] - a_next[i, j]
        else
            c[i, j] = current_w[i, j] + current_e[i, j] * current_a[i, j] * (1 + rr) + current_e[i, j] * current_p[i, j] * (1 - delta) * current_d[i, j] - a_next[i, j] - current_p[i, j] * d_next[i, j]
        end
    end
