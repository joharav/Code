...
function plot_simulated_d_and_a_by_state(simdata)
    ...
    for ei in unique_e, yi in unique_y
        ...
        aa_vals = [aa[I] for I in idx]

        histogram(d_vals, bins=50, normalize=true,
                  xlabel="Durables", ylabel="Density",
                  title="Durables | e=$ei, y=$yi", legend=false)
        savefig(joinpath(output_dir, "d_dist_e$(ei)_y$(yi).png"))

        histogram(a_vals, bins=50, normalize=true,
                  xlabel="Foreign assets a", ylabel="Density",
                  title="a | e=$ei, y=$yi", legend=false)
        savefig(joinpath(output_dir, "a_dist_e$(ei)_y$(yi).png"))

        histogram(aa_vals, bins=50, normalize=true,
                  xlabel="Local assets aa", ylabel="Density",
                  title="aa | e=$ei, y=$yi", legend=false)
        savefig(joinpath(output_dir, "aa_dist_e$(ei)_y$(yi).png"))
    end
end
