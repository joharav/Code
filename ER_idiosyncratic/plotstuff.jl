...
function plotstuff(vee::Array{Float64, 4}, apol::Array{Int64, 4}, dpol::Array{Int64, 4},  cpol::Array{Float64, 4}, g::NamedTuple)
    ...
    iy = Int(floor(sz.ny / 2))  # Fix a productivity index
    ...
    for ie in 1:sz.ne
        ...
        vf = permutedims(vee[ie, iy, :, :], (1, 2))   # (nd, na)
        aa = permutedims(apol[ie, iy, :, :], (1, 2))
        dd = permutedims(dpol[ie, iy, :, :], (1, 2))
        cc = permutedims(cpol[ie, iy, :, :], (1, 2))
     ...
     for ie in 1:sz.ne
         for d_level in [Int(floor(sz.nd / 3)), Int(floor(2 * sz.nd / 3))]
             ...
             plot!(plt, a, dpol[ie, iy, :, d_level], label="e=$ie")
     ...
     for ie in 1:sz.ne
         for d_level in [Int(floor(sz.nd / 3)), Int(floor(2 * sz.nd / 3))]
             ...
             plot!(plt, a, apol[ie, iy, :, d_level], label="e=$ie")
     ...
     for ia in a_idx, id in d_idx
         ...
         plotA = plot(e_vals, apol[:, iy, ia, id], xlabel="Exchange Rate", ylabel="Asset Policy", title="a' vs e (a=$a_val, d=$d_val)", marker=:circle, label=false)
         plotD = plot(e_vals, dpol[:, iy, ia, id], xlabel="Exchange Rate", ylabel="Durable Policy", title="d' vs e (a=$a_val, d=$d_val)", marker=:square, label=false)
