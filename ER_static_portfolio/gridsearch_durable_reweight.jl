...
# ==========================================================================
const WFILE_EST = joinpath(kst.DATA_DIR, "W_pick6_prio125_diag.csv")  # <-- your new file

const DAT = let
    dm = vec(collect(readdlm(kst.MOMS_FILE)))
    W0 = collect(readdlm(WFILE_EST))              # <-- use prioritized W here
...
# ==========================================================================
function save_ckpt!(x_best, f_best; path="Output/smm_ckpt_4D_prio125.jls")
    isdir(dirname(path)) || mkpath(dirname(path))
    serialize(path, (; x_best=copy(x_best), f_best=f_best, t=time()))
end

function load_ckpt(path="Output/smm_ckpt_4D_prio125.jls")
