...
#SBATCH --job-name=run_disaster_counterfactual
...
echo "Starting run_disaster_counterfactual.jl at $(date)"
echo "Host: $(hostname)  Threads: ${JULIA_NUM_THREADS}"

julia --threads=${JULIA_NUM_THREADS} -O3 run_disaster_counterfactual.jl
