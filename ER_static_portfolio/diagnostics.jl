# ==========================================================================
# 4D MODEL: Diagnostics for SMM identification
# ==========================================================================

using LinearAlgebra, Statistics, Printf
using DelimitedFiles

# Parameter names for 4D model
const Fd_sym = Symbol("F^d")
const Ft_sym = Symbol("F^t")

const PAR_NAMES = [:nu, Fd_sym, :kappa, :chi, Ft_sym]
...
# Simulated moments
sim_mom(p::Vector{Float64}) = fcn(p)
...
function run_profiles(θhat; grids::Dict{Symbol,AbstractVector}, outcsv_dir="Output/profiles")
...
function moment_sensitivity(θhat; j::Symbol, grid::AbstractVector)
    ...
    Mgrid = reduce(vcat, (permutedims(m) for m in Ms))
...
function run_moment_sensitivities(θhat; grids::Dict{Symbol,AbstractVector},
                                 outcsv_dir="Output/sens")
    isdir(outcsv_dir) || mkpath(outcsv_dir)
    mnames = ["duration_mean", "dwealth_mean", "dwealth_var", 
...
# ==========================================================================
# Sanity checks for 4D model
# ==========================================================================
function check_sanity(θ::Vector{Float64}; 
                     target_adj_rate=(0.05, 0.15),
                     target_dollar_share=(0.40, 0.70))
    ...
    # Metrics
    adj_rate = mean(adj)
    durable_share = mean(vec(d) .> 0.0)
    
    a_loc = aa
    a_fx = ex .* a
    a_eff = a_loc .+ a_fx .+ 1e-12
    dollar_share = mean(vec(a_fx ./ a_eff))
    
    # Check targets
    ok_adj = target_adj_rate[1] ≤ adj_rate ≤ target_adj_rate[2]
    ok_dollar = target_dollar_share[1] ≤ dollar_share ≤ target_dollar_share[2]
...
# ==========================================================================
function print_diagnostics(θ::Vector{Float64})
    sanity = check_sanity(θ)
    
    println("\n" * "="^50)
    println("4D MODEL DIAGNOSTICS")
    println("="^50)
    ...
    @printf("  Durable share:   %.4f\n", sanity.durable_share)
    @printf("  Dollar share:    %.4f\n", sanity.dollar_share)
    
    println("\nTarget checks:")
    println("  Adjustment rate in [0.05, 0.15]: ", sanity.ok ? "✓" : "✗")
    println("  Dollar share in [0.40, 0.70]:    ", sanity.ok ? "✓" : "✗")
